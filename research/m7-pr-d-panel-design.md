...
two, then one, then none blocking, then two, then one, then none; see the
implementation record), and merged as PR #19. The
...
- Mutation check (shipped): twenty-six mutants — no canonical order, no
  ...
  registration and at admission, a boundary compared as models at authoring,
  registration, and admission, and two ways of not re-deriving a result —
