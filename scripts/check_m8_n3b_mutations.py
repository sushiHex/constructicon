...
MUTANTS = (
    ...
    (
        "17 the pump rechecks liveness and control after every read",
        RELAY + "_pump",
        "self._admit(loop)",
        "pass",
        E + "test_control_lost_on_an_established_stream_forwards_nothing_more",
    ),
    (
    ...
    (
        "40 every upstream has zero linger from creation, so each close discards its queue",
        EGRESS + "_upstream",
    ...
    (
        "45 an established stream checks control, not only liveness",
        RELAY + "_pump",
        "self._admit(loop)",
        "self._require_live(loop)",
        E + "test_control_lost_on_an_established_stream_forwards_nothing_more",
    ...
    (
        "46 a destination pins a routable address",
        EGRESS + "EgressDestination.__post_init__",
        "if not _routable(ipaddress.ip_address(self.address)):",
        "if False:",
        E + "test_a_host_local_or_zoned_address_is_never_admissible[loopback]",
    ),
    (
        "47 a destination pins an address without a zone",
        EGRESS + "EgressDestination.__post_init__",
        'if "%" in self.address:',
        "if False:",
        E + "test_a_host_local_or_zoned_address_is_never_admissible[zone]",
    ),
    (
        "48 a routable address is global",
        EGRESS + "_routable",
        "address.is_global and not",
        "not",
        E + "test_a_host_local_or_zoned_address_is_never_admissible[private]",
    ),
    (
        "49 a routable address is not multicast",
        EGRESS + "_routable",
        "not address.is_multicast and not address.is_reserved",
        "not address.is_reserved",
        E + "test_a_host_local_or_zoned_address_is_never_admissible[multicast]",
    ),
    (
        "50 a routable address is not reserved",
        EGRESS + "_routable",
        " and not address.is_reserved",
        "",
        E + "test_a_host_local_or_zoned_address_is_never_admissible[reserved-v6]",
    ),
    (
        "51 a routable IPv6 address is not site-local",
        EGRESS + "_routable",
        "and address.is_site_local)",
        "and False)",
        E + "test_a_host_local_or_zoned_address_is_never_admissible[site-local]",
    ),
    (
        "52 a CONNECT head has exactly three request-line tokens",
        EGRESS + "parse_connect",
        "len(parts) != 3 or ",
        "",
        E + "test_connect_parser_refusals[extra-token]",
    ),
    (
        "53 only the CONNECT method is parsed",
        EGRESS + "parse_connect",
        'parts[0] != "CONNECT" or ',
        "",
        E + "test_connect_parser_refusals[method]",
    ),
    (
        "54 only HTTP/1.1 is parsed",
        EGRESS + "parse_connect",
        ' or parts[2] != "HTTP/1.1"',
        "",
        E + "test_connect_parser_refusals[version]",
    ),
    (
        "55 the CONNECT port has one decimal spelling",
        EGRESS + "parse_connect",
        "_PORT.fullmatch(port) is None or ",
        "",
        E + "test_connect_parser_refusals[zero-padded-port]",
    ),
    (
        "56 the CONNECT port is bounded",
        EGRESS + "parse_connect",
        " or int(port) > 65535",
        "",
        E + "test_connect_parser_refusals[port-over-bound]",
    ),
    (
        "57 the CONNECT host follows the DNS grammar",
        EGRESS + "parse_connect",
        "or not _is_host(host)",
        "or False",
        E + "test_connect_parser_refusals[userinfo]",
    ),
    (
        "58 an IP-literal CONNECT host is its own denial",
        EGRESS + "parse_connect",
        'if host.startswith("[") or _is_literal(host):',
        'if host.startswith("["):',
        E + "test_connect_parser_refusals[literal]",
    ),
    (
        "59 a bracketed CONNECT host is an IP-literal denial",
        EGRESS + "parse_connect",
        'host.startswith("[") or ',
        "",
        E + "test_connect_parser_refusals[bracketed]",
    ),
    (
        "60 a CONNECT head must be terminated",
        EGRESS + "parse_connect",
        r'if not text.endswith("\r\n\r\n"):',
        "if False:",
