...
async def measure(image, guard_root, model, *, files=None, config=None, arguments=()):
    ...
    async with placement(image, model=model, scenario=PROBE_PROMPT, tool=scenario.tool,
        ...
        observations, result = await observe(
            ...
            files={MARKER: "absent", **(files or {})}, arguments=arguments,
        )
...
async def test_named_profile_is_loaded_only_when_explicitly_selected(
    placement_image, tmp_path, selected,
):
    ...
    record = await measure(
        ...
        arguments=("--profile", "fixture") if selected else (),
    )
    assert record["config"]["config"]["model"] == (MODELS[1] if selected else MODELS[0])
    # thread/start still explicitly chooses the probe's model; profile discovery
    # is independently asserted through config/read rather than inferred from it.
...
async def test_user_hook_discovery_trust_and_disable_have_execution_controls(
    ...
    for enabled in (True, False):
        ...
        # inspected. This is a positive execution control, not the safe recipe.
        selected = config + (
        ...
        assert marker(checked) == ("hook" if enabled else "absent")
